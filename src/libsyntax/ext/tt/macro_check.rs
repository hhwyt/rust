    ...
    for tt in tts {
        match (state, tt) {
                ...
                if del.delim == DelimToken::Brace =>
            {
                ...
                let rest = check_nested_macro(sess, node_id, &del.tts, &nested_macros, valid);
...
fn check_nested_macro(
    ...
    let mut i = 0;
    loop {
        // We expect 3 token trees: `(LHS) => {RHS}`. The semicolon is checked after.
        if i + 2 >= n
            || !tts[i].is_delimited()
        ...
        // Since the last semicolon is optional, we increment our checked position by how many token
        // trees we already checked (the 3 above) before checking for the semicolon.
        i += 3;
        if i == n || !tts[i].is_token(&TokenKind::Semi) {
            break;
        }
